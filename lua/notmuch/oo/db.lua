...
local messages = require("../notmuch.query")
...
function M:find_message(mid)
	return messages:new(nm.db_find_message(self.db, mid))
end

function M:find_message_by_filename(filename)
	return messages:new(nm.db_find_message_by_filename(self.db, filename))
end
